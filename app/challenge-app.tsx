...
const localToday = () => {
  ...
  if (today < "2026-08-01") return "2026-08-01";
  if (today > "2026-09-19") return "2026-09-19";
  return today;
};
    ...
    <main className="login-shell">
      <section className="login-story">
        <div className="login-story__top">
          ...
          <p>Log every session as it happens. Your weight stays private during the week, then the family reveal lands together on Saturday.</p>
          <div className="login-story__facts"><span><CalendarDays size={18} /> 1 Aug — 19 Sep</span><span><Trophy size={18} /> $50 per kg*</span><span><Dumbbell size={18} /> 30+ min sessions</span></div>
        </div>
...
function SessionModal({ participant, onClose, onSave, saving }: {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}>
    <section className="check-in-modal session-modal" role="dialog" aria-modal="true" aria-labelledby="session-heading" onMouseDown={(e) => e.stopPropagation()}>
      ...
      <div className="privacy-callout"><LockKeyhole size={19} /><div><strong>Your weight stays private</strong><span>If you add it, only you can see it until the weekly Saturday reveal.</span></div></div>
      <form className="check-in-form" onSubmit={submit}>
        <div className="form-row">
          <label className="field"><span>Date</span><div className="input-with-suffix"><input type="date" min="2026-08-01" max="2026-09-19" value={date} onChange={(e) => setDate(e.target.value)} required /></div></label>
    ...
    <main className="dashboard">
      {section === "overview" && <>
        <section className="challenge-hero">
          ...
          <div className="challenge-hero__progress reveal-card"><div className="day-orbit"><EyeOff size={18} /><strong>W{currentWeek}</strong><small>PRIVATE</small></div><div className="hero-progress-copy"><strong>Next family reveal</strong><span>Saturday {formatChallengeDate(nextReveal)} · everyone&apos;s results unlock together</span><div className="progress-track"><i style={{ width: "72%" }} /></div></div></div>
          <Image src="/images/davo-hero-mascot.webp" alt="" className="dashboard-mascot" width={1254} height={1254} unoptimized />
        </section>
        <section className="stats-grid">
          <StatCard icon={<Scale size={22} />} label="My latest weight" value={latestPrivateWeight ? `${latestPrivateWeight.weightKg.toFixed(1)} kg` : "Not logged"} detail="Private until Saturday reveal" tone="yellow" />
        ...
        <section className="privacy-banner"><LockKeyhole size={22} /><div><strong>Your in-week weight is for your eyes only</strong><span>The activity feed shows that you trained, but never shows the weight you entered. Weekly changes are revealed after the Saturday cutoff.</span></div><span className="privacy-badge"><Eye size={15} /> You only</span></section>
      ...
      {section === "standings" && <div className="standings-page">
        <section className="standings-heading"><div><span className="eyebrow">Released results only</span><h1>Challenge standings</h1><p>Weight rankings update together after each Saturday reveal. Activity totals stay live all week.</p></div><div className="standings-heading__badge"><EyeOff size={24} /><span>Next reveal<strong>{formatChallengeDate(nextReveal)}</strong></span></div></section>
